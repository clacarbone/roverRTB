...
//    for (i=0;i<OA_radial_size-1;i++)
//    {
//        len=sprintf(mystr,"%03d,",OA_radial_map[i].intensity);
//        write(output_pty,mystr,len);
//    }
//    len=sprintf(mystr,"%03d,END\n\r",OA_radial_map[OA_radial_size].intensity);
//    write(output_pty,mystr,len);
    fclose(output_pty);
    ...
    {
        long count_left=0, count_right=0, result;
        ...
        /*result = MIN(count_left,count_right);
        if (count_left > 0)
            if (OA_radial_map[index-count_left].validity == 1)
                control_values.heading = (RTB_FLOAT_TYPE)((index * 5) - 5 * (count_left+1));
        if (result == count_right)
            control_values.heading = (RTB_FLOAT_TYPE)((index * 5) + 5 * (count_right+1));
        
        if (control_values.heading < 0)
            control_values.heading += 360;
        if (control_values.heading > 360)
            control_values.heading -= 360;*/
